"""MCP tool implementations for Kanka operations."""

import logging
from typing import Any

from .operations import get_operations
from .types import (
    CheckEntityUpdatesResult,
    CreateEntityResult,
    CreatePostResult,
    DeleteEntityResult,
    DeletePostResult,
    GetEntityResult,
    UpdateEntityResult,
    UpdatePostResult,
)

logger = logging.getLogger(__name__)


async def handle_find_entities(**params: Any) -> dict[str, Any]:
    """Find entities by search and/or filtering."""
    operations = get_operations()
    return await operations.find_entities(
        query=params.get("query"),
        entity_type=params.get("entity_type"),
        name=params.get("name"),
        name_exact=params.get("name_exact", False),
        name_fuzzy=params.get("name_fuzzy", False),
        type=params.get("type"),
        tags=params.get("tags", []),
        date_range=params.get("date_range"),
        include_full=params.get("include_full", True),
        page=params.get("page", 1),
        limit=params.get("limit", 25),
        last_synced=params.get("last_synced"),
    )


async def handle_create_entities(**params: Any) -> list[CreateEntityResult]:
    """Create one or more entities."""
    entities = params.get("entities", [])
    operations = get_operations()
    return await operations.create_entities(entities)


async def handle_update_entities(**params: Any) -> list[UpdateEntityResult]:
    """Update one or more entities."""
    updates = params.get("updates", [])
    operations = get_operations()
    return await operations.update_entities(updates)


async def handle_get_entities(**params: Any) -> list[GetEntityResult]:
    """Retrieve specific entities by ID."""
    entity_ids = params.get("entity_ids", [])
    include_posts = params.get("include_posts", False)
    operations = get_operations()
    return await operations.get_entities(entity_ids, include_posts)


async def handle_delete_entities(**params: Any) -> list[DeleteEntityResult]:
    """Delete one or more entities."""
    entity_ids = params.get("entity_ids", [])
    operations = get_operations()
    return await operations.delete_entities(entity_ids)


async def handle_create_posts(**params: Any) -> list[CreatePostResult]:
    """Create posts on entities."""
    posts = params.get("posts", [])
    operations = get_operations()
    return await operations.create_posts(posts)


async def handle_update_posts(**params: Any) -> list[UpdatePostResult]:
    """Update existing posts."""
    updates = params.get("updates", [])
    operations = get_operations()
    return await operations.update_posts(updates)


async def handle_delete_posts(**params: Any) -> list[DeletePostResult]:
    """Delete posts from entities."""
    deletions = params.get("deletions", [])
    operations = get_operations()
    return await operations.delete_posts(deletions)


async def handle_check_entity_updates(**params: Any) -> CheckEntityUpdatesResult:
    """Check which entity_ids have been modified since last sync."""
    entity_ids = params.get("entity_ids", [])
    last_synced = params.get("last_synced")
    if not last_synced:
        raise ValueError("last_synced parameter is required")
    operations = get_operations()
    return await operations.check_entity_updates(entity_ids, last_synced)


async def handle_manage_relations(**params: Any) -> list[dict[str, Any]]:
    """Manage entity relations (create/update/delete/list)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_relations(actions)


async def handle_manage_attributes(**params: Any) -> list[dict[str, Any]]:
    """Manage entity attributes (create/update/delete/list/bulk_patch)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_attributes(actions)


async def handle_manage_organisation_members(**params: Any) -> list[dict[str, Any]]:
    """Manage organisation members (create/update/delete/list)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_organisation_members(actions)


async def handle_manage_map_markers(**params: Any) -> list[dict[str, Any]]:
    """Manage map markers (create/update/delete/list)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_map_markers(actions)


async def handle_manage_map_groups(**params: Any) -> list[dict[str, Any]]:
    """Manage map groups (create/update/delete/list)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_map_groups(actions)


async def handle_manage_map_layers(**params: Any) -> list[dict[str, Any]]:
    """Manage map layers (create/update/delete/list)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_map_layers(actions)


async def handle_manage_calendar_reminders(**params: Any) -> list[dict[str, Any]]:
    """Manage calendar reminders (events on calendar dates)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_calendar_reminders(actions)


async def handle_manage_timeline_eras(**params: Any) -> list[dict[str, Any]]:
    """Manage timeline eras (create/update/delete/list)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_timeline_eras(actions)


async def handle_manage_timeline_elements(**params: Any) -> list[dict[str, Any]]:
    """Manage timeline elements (create/update/delete/list)."""
    actions = params.get("actions", [])
    operations = get_operations()
    return await operations.manage_timeline_elements(actions)
